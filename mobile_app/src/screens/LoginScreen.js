...
import {
  ...
  ActivityIndicator, KeyboardAvoidingView, Platform, ScrollView, Modal, ToastAndroid, Alert
} from 'react-native';
import { useAuth } from '../context/AuthContext';
import { loginUser, sendOtpToEmail, executePasswordReset } from '../services/authService';
...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const message = err.response?.data?.message || 'Login rejected. Verify credentials.';
  ...
  const handleExecuteReset = async () => {
    ...
    setModalLoading(true);
    try {
      const res = await executePasswordReset(recoveryEmail.trim(), recoveryOtp.trim(), newPassword.trim());
      showNotification('Password updated successfully! Proceed to login.');
      closeRecoveryModal();
    } catch (err) {
      const msg = err.response?.data?.message || 'Reset rejected. Token expired.';
      showNotification(msg);
    } finally {
      setModalLoading(false);
    }
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.headerSection}>
          <View style={styles.logoBadge}>
            <Ionicons name="leaf" size={36} color="#2e7d32" />
          </View>
          <Text style={styles.title}>AgriGuard</Text>
          <Text style={styles.subtitle}>Smart AI Disease Diagnostics & Management</Text>
        </View>
        ...
        <View style={styles.cardContainer}>
          ...
          <TouchableOpacity onPress={() => setForgotModalVisible(true)} style={styles.forgotLinkContainer}>
          ...
          <TouchableOpacity style={styles.button} onPress={handleLogin} disabled={loading}>
            {loading ? (
              ...
              <View style={styles.buttonContent}>
                ...
                <Ionicons name="log-in-outline" size={18} color="#fff" style={styles.buttonArrow} />
              </View>
            )}
          </TouchableOpacity>
        </View>
      ...
      {/* ========================================================
          PASSWORD RECOVERY MODAL WINDOW SHEET
         ======================================================== */}
      <Modal visible={forgotModalVisible} animationType="slide" transparent={true} onRequestClose={closeRecoveryModal}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContentCard}>
            
            {/* Header with Left-Aligned Cross Icon */}
            <View style={styles.modalHeaderRow}>
              <TouchableOpacity onPress={closeRecoveryModal} style={styles.modalLeftClose}>
            ...
            {!isRecoveryOtpSent ? (
              <View>
                <Text style={styles.modalInstruction}>Enter your account email below. We will send a secure 6-digit code to authorize a password change.</Text>
                ...
                <TouchableOpacity style={styles.button} onPress={handleRequestRecoveryOTP} disabled={modalLoading}>
                  {modalLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Get Reset Code</Text>}
                </TouchableOpacity>
              </View>
            ) : (
              <View>
                <Text style={styles.modalInstruction}>Type the code sent to your email inbox and establish your new login password parameters.</Text>
                ...
                {/* Modal Password Input with View Toggle option */}
                <View style={styles.inputWrapper}>
                  ...
                  />
                  <TouchableOpacity onPress={() => setShowModalPassword(!showModalPassword)} style={styles.eyeIcon}>
                    <Ionicons name={showModalPassword ? "eye-off-outline" : "eye-outline"} size={20} color="#888" />
                  </TouchableOpacity>
                </View>

                <TouchableOpacity style={styles.button} onPress={handleExecuteReset} disabled={modalLoading}>
                  {modalLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Save New Password</Text>}
                </TouchableOpacity>
              </View>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  logoBadge: { backgroundColor: '#e8f5e9', padding: 16, borderRadius: 20, marginBottom: 12 },
  title: { fontSize: 32, fontWeight: '800', color: '#2e7d32', letterSpacing: 0.5 },
  ...
  eyeIcon: { padding: 4 },
  forgotLinkContainer: { alignSelf: 'flex-end', marginBottom: 20, marginTop: -4, padding: 2 },
  forgotLinkText: { color: '#2e7d32', fontSize: 13, fontWeight: '600' },
  button: { backgroundColor: '#2e7d32', height: 52, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  buttonContent: { flexDirection: 'row', alignItems: 'center' },
  buttonText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  buttonArrow: { marginLeft: 8 },
  linkContainer: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 24, padding: 8 },
  ...
  modalContentCard: { backgroundColor: '#fff', borderRadius: 20, padding: 24, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10, elevation: 5 },
  modalHeaderRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 18 },
  modalLeftClose: { marginRight: 12, padding: 2 },
  modalTitle: { flex: 1, fontSize: 20, fontWeight: '700', color: '#2e7d32', textAlign: 'left' },
  modalInstruction: { fontSize: 13, color: '#666', marginBottom: 16, lineHeight: 18 }
});